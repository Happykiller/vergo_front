...
const Training_create: React.FC = () => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      try {
        const response = await inversify.createTrainingUsecase.execute(tmp);
        if (response.error) {
          throw new Error(response.error);
        }
        flash.open(t('training_create.create_success'));
        navigate({
          pathname: '/trainings'
        });
      } catch(e:any) {
        flash.open(t('training_create.create_fail'));
      }
    } catch(e:any) {
      flash.open(t('training_create.json_fail'));
    }
  }
